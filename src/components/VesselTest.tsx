 ...
 */
const VesselTest = () => {
  const { animatedVessels } = useVesselLocation();
  ...
  return (
    <View className="absolute top-20 left-4 bg-white/80 p-4 rounded z-50 max-w-80">
      ...
      <Text className="text-sm text-black mb-1">
        Total Vessels: {animatedVessels.length}
      </Text>
      {animatedVessels.slice(0, 3).map((vessel) => (
      ...
      {animatedVessels.length > 3 && (
        <Text className="text-xs text-gray-600">
          ... and {animatedVessels.length - 3} more vessels
        </Text>
      )}
    </View>
  );
